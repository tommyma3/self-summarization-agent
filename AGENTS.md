...
- An output inside unresolved thinking, with ambiguous syntax, without a supported sampled end-of-turn token, or with an unclosed or wrongly closed end-of-thinking token is treated as malformed: retain the raw completion, penalize, and stop. On the TITO path only the byte-exact `</think>` closes the block; known typos such as `</thinking>` and case/whitespace variants are classified as malformed (with a `format_anomaly` record distinguishing `unclosed_thinking` from `wrongly_closed_thinking`), never repaired or routed. Sampled tokens are never repaired. This applies to every generation kind: tool actions, final answers, forced answers, and summaries. Legacy message-based collection retains its strict unclosed-thinking guard for tool actions.
