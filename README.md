...
The executor is intentionally custom instead of using OpenRLHF's default multi-turn executor because this project must not append model CoT to later prompts. Tool-call turns are generated by the policy, parsed, and converted into normalized visible tool calls plus tool results. Only summary and final-answer spans are returned as trainable action ranges, and they share the same terminal reward from the existing LLM judge.
