...
### Optional OpenRLHF agent training path

The OpenRLHF integration keeps the existing BrowseComp-Plus runtime semantics but delegates distributed RL training to OpenRLHF. The custom executor lives at:

- [src/self_summarization_agent/openrlhf_agent.py](/D:/M/CS/self-summarization-agent/src/self_summarization_agent/openrlhf_agent.py)

The executor is intentionally custom instead of using OpenRLHF's default multi-turn executor because this project must not append model CoT to later prompts. Tool-call turns are generated by the policy, parsed, and converted into normalized visible tool calls plus tool results for later prompts. The trainable tool-call and final-answer spans keep the full generated output, including thinking sections. Tool-call, summary, and final-answer spans are returned as trainable action ranges, and completed episodes assign the same terminal reward to all of them from the existing LLM judge.
...
python -m self_summarization_agent.openrlhf_dataset \
  ...
  --output artifacts/openrlhf/train_prompts.jsonl

python -m self_summarization_agent.openrlhf_judge_server \
  --config configs/train/default.yaml \
  --host 127.0.0.1 \
  --port 8765

TRAIN_CONFIG=configs/train/default.yaml \
PROMPT_DATA=artifacts/openrlhf/train_prompts.jsonl \
JUDGE_URL=http://127.0.0.1:8765/judge \
bash scripts/train_openrlhf_agent.sh
```

The OpenRLHF worker reads `SELF_SUMMARIZATION_OPENRLHF_CONFIG`; the launch script sets it automatically. Set `SELF_SUMMARIZATION_JUDGE_URL` to keep the judge in a separate process from actor rollout/training memory.
