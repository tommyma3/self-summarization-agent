...
Each training interval begins with the system instructions and either the original user request or the latest compressed state wrapped in `<summary>...</summary>`. On the OpenAI-compatible path, `search`, `get_document`, and `finish` are native function tools and tool results are linked `role: tool` messages. Summary boundaries are terminal runtime-control `user` messages, while forced-answer boundaries remain terminal runtime-control `system` messages; both are appended without changing the earlier system/tool prefix. API `tool_choice` disables tools for a summary or requires `finish` for a forced answer. The generated summary remains a normal assistant message and must contain `<summary>...</summary>`. The runtime stores its extracted body for metrics and diagnostics, then rewraps that body only when constructing the next interval's initial user message. The runtime preserves the structured assistant reasoning, tool calls, tool-call IDs, and full raw sampled completion.
...
- A successful compaction starts a fresh interval with only the system instructions and `<summary>...</summary>`-wrapped compressed state; no raw tail is retained.
