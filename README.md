...
- exact `vllm_offline` training generations retain raw sampled-token logprobs and assemble v6 training caches directly from authoritative collection IDs and assistant masks before the policy process exits
...
- `cache_step` preserves complete rollout-native v6 caches without loading a model; when fallback rescoring is required, a GPU-0 policy scorer starts only after the judge process exits. Resume preserves completed v6 rows and regenerates older cache versions as v6
...
- training checkpoints are written through `.iteration-NNNNN.incomplete`; a complete staged checkpoint is promoted on resume, while an incomplete one is retained as `.stale-NNN` before the update is retried
...
The value trainer exploits causal prefix equivalence for the frozen `V_old` pass: it evaluates only through the exact state anchor. During the joint update, Qwen's `logits_to_keep` path materializes vocabulary logits only for the state anchor and trainable assistant-token positions. This optimization is gated by `training.value.enabled`; ordinary group-relative GRPO keeps its existing backend and loss path.

The policy remains a normal Hugging Face checkpoint for vLLM. The value head is saved beside it as `compaction_value_head.safetensors` plus `compaction_value_config.json`; value-enabled resumes after iteration zero require both policy and sidecar state. The preset uses `training.backend: verl_ray` with `training.verl.worker_backend: transformers`, because the native verl FSDP actor does not currently expose the required hidden state or sidecar checkpoint hooks. `runtime.train_update_timeout_seconds` can give this update a separate wall-clock limit without weakening collection and judge timeouts; progress is written atomically to `iteration-NNNNN.train-progress.json`.
